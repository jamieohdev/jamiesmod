...
public class AncientRootStructure  extends Structure {
    ...
    public AncientRootStructure(Structure.Config config, HeightProvider height) {
        super(config);
        this.height = height;
    }
    ...
    public Optional<StructurePosition> getStructurePosition(Structure.Context context) {
        ...
        while(l > k) {
            ...
            if (blockState.isAir() && (blockState2.isOf(Blocks.MOSS_BLOCK) ||
                    blockState2.isSideSolidFullSquare(EmptyBlockView.INSTANCE, mutable.setY(l),
                            Direction.UP))) {
                break;
            }
        }
        ...
        if (l <= k) {
            ...
            return Optional.of(new Structure.StructurePosition(blockPos, (holder) -> {
                AncientRootGenerator.addPieces(context.structureTemplateManager(), holder, chunkRandom, blockPos);
            }));
        }
    }
